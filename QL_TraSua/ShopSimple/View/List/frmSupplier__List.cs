...
namespace ShopSimple.View.List
{
    public partial class frmSupplier__List : Form
    {
        ...
        private void dgvList_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                ShowMessagebox.Error("Chưa chọn nhà cung cấp!");
                return;
            }
        ...
        private void listLoad()
        {
            ...
            {
                setList();
                lblResult.Text = "Kết quả tìm thấy: " + new bSupplier().GetCount(tbSearch.Text.Trim()).ToString();
            }
            catch (Exception ex)
            {
        ...
        {
            IEnumerable<Supplier> data = new bSupplier().GetList(tbSearch.Text.Trim());
        ...
        private void deleteRows()
        {
            ...
            if (ShowMessagebox.Question($"Bạn có muốn xoá nhà cung cấp {String.Join(",", listSelectRowFromDGV.ToArray())} không?") == DialogResult.No)
                return;

            List<Supplier> bak = new List<Supplier>();

            foreach (var i in listSelectRowFromDGV)
            {
                bool rs = new bSupplier().Delete(i);

                if (!rs)
                {
                    bak.ForEach(e => new bSupplier().Add(e));
                    ShowMessagebox.Error($"Không thể xoá {i}: {new bSupplier().GetDetail(i).Name}!");
                    return;
                }

                bak.Add(new bSupplier().GetDetail(i));
            }
        }
        ...
        private void openForm(bool isView, bool isAdd)
        {
            if (isAdd)
            {
                using (var frm = new frmSupplier__Detail(isAdd))
                {
                    frm.ShowDialog();
            ...
            using (var frm = new frmSupplier__Detail(code__selected, isView))
            {
                frm.ShowDialog();
                if (frm.DialogResult == DialogResult.Yes)
                    listLoad();
