...
namespace ShopSimple.View.List
{
    public partial class frmAccount__List : Form
    {
        ...
        private void dgvList_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                ShowMessagebox.Error("Chưa chọn nhân viên!");
                return;
            }
        ...
        private void listLoad()
        {
            ...
            {
                setList();
                lblResult.Text = "Kết quả tìm thấy: " + new bUser().GetCount(tbSearch.Text.Trim()).ToString();
            }
            catch (Exception ex)
            {
        ...
        {
            IEnumerable<User> data = new bUser().GetList(tbSearch.Text.Trim());
        ...
        private void deleteRows()
        {
            ...
            if (ShowMessagebox.Question($"Bạn có muốn xoá nhân viên {String.Join(",", listSelectRowFromDGV.ToArray())} không?") == DialogResult.No)
                return;

            List<User> bak = new List<User>();

            foreach (var i in listSelectRowFromDGV)
            {
                bool rs = new bUser().Delete(i);

                if (!rs)
                {
                    bak.ForEach(e => new bUser().Add(e));
                    ShowMessagebox.Error($"Không thể xoá {i}: {new bUser().GetDetail(i).Name}!");
                    return;
                }

                bak.Add(new bUser().GetDetail(i));
            }
        }
