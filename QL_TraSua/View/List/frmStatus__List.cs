...
namespace ShopSimple.View.List
{
    public partial class frmStatus__List : Form
    {
        ...
        private void dgvList_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                ShowMessagebox.Error("Chưa chọn tình trạng!");
                return;
            }
        ...
        private void listLoad()
        {
            ...
            {
                setList();
                lblResult.Text = "Kết quả tìm thấy: " + new bStatus().GetCount(tbSearch.Text.Trim()).ToString();
            }
            catch (Exception ex)
            {
        ...
        {
            IEnumerable<Status> data = new bStatus().GetList(tbSearch.Text.Trim());
            ...
            dgvList.Rows.AddRange(data.Select((i, count) => new DataGridViewRow
            {
                Cells =
                {
                    ...
                    new DataGridViewTextBoxCell { Value = i.StatusCode },
        ...
        private void deleteRows()
        {
            ...
            if (ShowMessagebox.Question($"Bạn có muốn xoá tình trạng {String.Join(",", listSelectRowFromDGV.ToArray())} không?") == DialogResult.No)
                return;

            List<Status> bak = new List<Status>();

            foreach (var i in listSelectRowFromDGV)
            {
                bool rs = new bStatus().Delete(i);

                if (!rs)
                {
                    bak.ForEach(e => new bStatus().Add(e));
                    ShowMessagebox.Error($"Không thể xoá {i}: {new bStatus().GetDetail(i).Name}!");
                    return;
                }

                bak.Add(new bStatus().GetDetail(i));
            }
        ...
        private void openForm(bool isView, bool isAdd)
        {
            if (isAdd)
            {
                using (var frm = new frmStatus_Detail(isAdd))
                {
                    frm.ShowDialog();
            ...
            using (var frm = new frmStatus_Detail(code__selected, isView))
            {
                frm.ShowDialog();
                if (frm.DialogResult == DialogResult.Yes)
                    listLoad();
